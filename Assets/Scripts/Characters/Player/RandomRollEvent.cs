...
public class RandomRollEvent : MonoBehaviour
{
    ...
    public float GetRandomFloatRoll(float minInclusive, float maxInclusive)
    {
        ...
        float luck = Mathf.Clamp(playerStats.playerLuck/10f, 0f, maximumFlatLuckIncrease); // 10f to make the number ingame bigger
    ...
    public int GetRandomIntRoll(int minInclusive, int maxExclusive)
    {
        ...
        int luck = (int)Mathf.Clamp(playerStats.playerLuck/10f, 0f, maximumFlatLuckIncrease); // 10f to make the number ingame bigger
